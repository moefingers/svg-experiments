 ...
 *   5  hexagon   (n=6, final)
 *
 * All shapes share a uniform topology of 6 vertices. At each stage, the
 * target positions are the regular n-gon's perimeter (with mid-edge
 * synthesized slots for n < 6). The vertex positions are driven by
 * per-vertex (x, y) springs that lerp toward the stage's target.
 *
 ...
 *
 * The rAF loop runs whenever any spring isn't settled, and stops once
...
export function UnfoldingBillboard({
  ...
  // Stage state — which stage the cascade is currently *targeting*. On
  // open, this cascades 0 → 1 → 2 → ... → FINAL_STAGE staggered by
  ...
  // initial position 0). Static initializer — don't read the ref during
  // render (React 19 lint complains).
  const [positions, setPositions] = useState<{ x: number; y: number }[]>(() =>
    Array.from({ length: TOPOLOGY_COUNT }, () => ({ x: 0, y: 0 })),
  );

  // Drive the stage cascade. On open, schedule timeouts to advance the
  // stage one at a time. On close, do the same in reverse.
  useEffect(() => {
    const timeouts: ReturnType<typeof setTimeout>[] = [];
    if (open) {
      // Cascade forward: 0 → 1 → 2 → ... → FINAL_STAGE.
      // Kickoff happens immediately for stage 1; later stages staggered.
      for (let s = 1; s <= FINAL_STAGE; s++) {
        timeouts.push(setTimeout(() => setStage(s), stageDelay * (s - 1)));
      }
    } else {
      // Cascade back: stage → stage - 1 → ... → 0.
      // Faster than open: half the stageDelay to feel snappier on exit.
    ...
    // We intentionally don't depend on `stage` — when open toggles, we
    // schedule from the current stage. Stage changes alone shouldn't
    // re-fire the schedule.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, stageDelay]);

  // When `stage` changes, update each spring's TARGET to the new stage's
  // vertex positions. The springs interpolate from their current values.
  useEffect(() => {
  ...
  // The loop runs whenever any spring is unsettled; once all are at rest,
  // we cancel the loop until the next state change kicks it back on.
  useEffect(() => {
  ...
  return (
    <div className={styles.host} style={style}>
      ...
      >
        ...
        {/* The morphing shape itself. Filled with translucent white for
            the glass surface, stroked with a brighter rim. */}
        <path
          d={pathStr}
        ...
        {/* Vertex pinpoints — small dots at each topology vertex. Visible
            during the unfold cascade, fade out once the shape settles
            (the rim stroke covers them). */}
        {positions.map((p, i) => (
          <circle
            key={i}
            cx={p.x}
      ...
      {/* Content overlay — rendered as HTML on top of the SVG, fading
          in once the shape is past stage 3 (square) so content doesn't
