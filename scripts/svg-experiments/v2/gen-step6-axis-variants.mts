/**
 * v2 / Step 6 — different rotation axes, exact per-keyframe bake.
 *
 * Step 5 demonstrated the cleanest case: pure tiltZ precess (rotation around
 * world-Z = screen-normal). Every bead's depth is invariant under that
 * rotation, so the apparent motion is a 2D screen-plane spin. That's why
 * Step 5 / 5b looks like "rotation on the Z axis" — because it literally is.
 *
 * This step covers axes whose rotations CHANGE bead depth, which is what
 * makes the bead shape morph (squish/expand) as it sweeps front-to-back.
 * For these, no primitive composition is exact — the bead's projected path
 * varies non-trivially across the cycle — so we bake the exact analytic
 * result per keyframe. Same rigor as Step 5, just with a richer orient(t).
 *
 * Axis patterns:
 *   wobble  — tiltX oscillates ±AMP° around the base tilt. tiltZ static.
 *   nutate  — fast tiltX oscillation + full tiltZ precess.
 *   tumble  — cos tiltX oscillation + full tiltZ precess (90° out of phase
 *             from nutate; gives a "rolling" feel).
 *   precess — tiltZ rotates, tiltX static. (Provided for parity with Step 5.)
 *   static  — no axis animation. Sanity case.
 *
 * Light is fixed in camera frame. No theta-walk in this step (beads do not
 * sweep their latitudes); the apparent bead motion comes purely from the
 * shell's orient animation. Light direction is recomputed per keyframe per
 * bead in the bead's current camera-frame position.
 *
 * Usage:
 *   pnpm tsx scripts/banner-experiments-v2/gen-step6-axis-variants.mts [pattern] [N] [dur] [Lx] [Ly] [Lz]
 */
...
const PATTERN = (process.argv[2] ?? "wobble").toLowerCase();
const N = Number(process.argv[3] ?? 150);
const DUR = process.argv[4] ?? "18s";
const N_KF = 60;
...
const P_LIGHT: [number, number, number] = [
  Number(process.argv[5] ?? DEFAULT_P[0]),
  Number(process.argv[6] ?? DEFAULT_P[1]),
  Number(process.argv[7] ?? DEFAULT_P[2]),
];
...
function orientAt(t: number): { tiltX: number; tiltZ: number } {
  ...
  switch (PATTERN) {
    ...
    default:
      throw new Error(`Unknown pattern "${PATTERN}". Valid: static, precess, wobble, nutate, tumble.`);
  }
}
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 320" width="800" height="320" role="img" aria-label="Step 6 — ${PATTERN}">
  <defs>
    ${gradients}
  ...
  <text class="label" x="40" y="40">v2 · Step 6</text>
  <text class="title" x="40" y="68">Axis variant · ${PATTERN}</text>
  <text class="sub"   x="40" y="92">N=${N}, dur=${DUR}. Exact per-keyframe bake. AMP=${AMP_DEG}°, cycles=${AXIS_CYCLES}.</text>
...
const outPath = resolve(
  projectRoot,
  `public/banner-experiments-v2/step6-${PATTERN}${nTag}${lightTag}.svg`,
);
writeFileSync(outPath, svg);
console.log(`Wrote ${(svg.length / 1024).toFixed(1)} KB → ${outPath}  (pattern=${PATTERN}, N=${N}, dur=${DUR}, P=${P_LIGHT.join(",")})`);
